...
import {
  Code2, ChevronRight, GitCommit, CheckCircle, Clock,
...
export function DashboardView({ onSwitchView }: { onSwitchView: () => void }) {
  const { projects, isOnline, totalLines, micState, simulateTask, addProject } = useAppStore();
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <header className="flex items-center justify-between border-b border-border px-4 py-3 md:px-6">
        <div className="flex items-center gap-2.5">
          {selectedProject && (
            <button onClick={() => { setSelectedProjectId(null); setExpandedTaskId(null); }} className="mr-1 text-muted-foreground hover:text-foreground transition-colors active:scale-95">
      ...
      {/* Stats strip */}
      <div className="flex items-center gap-6 border-b border-border px-4 py-2.5 md:px-6">
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            <span className="font-semibold text-foreground">{totalCompleted}</span> tasks
          </span>
        </div>
        <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-1 flex items-center gap-3">
                          <span className="text-[11px] text-muted-foreground">{done}/{total} tasks done</span>
                          {latestTask && (
                            <span className="text-[11px] text-muted-foreground truncate">· {timeAgo(latestTask.timestamp)}</span>
                          )}
                        </div>
                        {/* Progress bar */}
                        {total > 0 && (
              ...
              >
                {selectedProject.tasks.length === 0 && (
                  <div className="py-16 text-center">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">Hit simulate to add one</p>
                  </div>
                )}
      ...
      <motion.button
        onClick={() => {
          if (selectedProject) {
            simulateTask(selectedProject.id);
          } else {
      ...
      >
        ...
        <span className="hidden md:inline text-sm font-medium">
          {selectedProject ? 'Simulate Task' : 'New Project'}
        </span>
      </motion.button>
    </div>
  );
